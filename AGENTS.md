...
## 基本方針
- `doc/mm166_ja.md` の問題文、`../knowledge`、関連する `../reference/<method>/*` を確認する。参照不能なら明示する。
...
- 新しい大近傍や重い探索方式は、10秒枠で直ちに不採用とせず、まず実験時だけ全体実行時間を延長して到達スコアと発火数を確認する。延長時に効果が確認できた場合は、候補削減・差分評価・前計算などで高速化してから10秒制限へ戻す。延長版と比較対象は同じ実行時間・seed・tester条件で評価し、提出版へ長時間設定を残さない。
