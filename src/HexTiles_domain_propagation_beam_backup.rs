        ...
        for &state in &by_dist[d] {
            ...
            for o in first_o..=last_o {
                let add_rot = if fixed[cell] {
                    ...
                    rotation_cost(orientation[cell], o)
                } as i16;
...
fn find_path_candidates(
    ...
    let reverse = build_reverse_estimate(W, grid, orientation, fixed, bonus, exits, target_exit);
...
fn analyze_board(
    ...
    let mut handled = vec![false; exits.len()];
    for source in 0..exits.len() {
        if handled[source] {
            continue;
        }
        let (mut cell, mut enter) = exits[source];
        let mut length = 0usize;
        let mut seen_tile = vec![false; grid.len()];
        let mut unique_tiles = 0usize;
        let mut path_bonus = 0usize;
        let mut target = usize::MAX;
        for _ in 0..=(grid.len() * 3) {
            length += 1;
            if !seen_tile[cell] {
                seen_tile[cell] = true;
                unique_tiles += 1;
                path_bonus += usize::from(bonus[cell]);
            }
            ...
            if !valid_cell(grid, W, nr, nc) {
                target = exit_id[cell * 6 + out] as usize;
                break;
            }
            cell = nr as usize * W + nc as usize;
            enter = opposite(out);
        }
        if target == usize::MAX {
            continue;
        ...
        handled[source] = true;
        handled[target] = true;
        if mate[source] == target {
            stats.matched += 1;
            stats.total_length += length;
            stats.bonus_crossings += path_bonus;
            stats.tile_revisits += length - unique_tiles;
            stats.total_path_score += (length * (path_bonus + 1)) as i64;
        }
    }
    stats.score =
...
fn main() {
    ...
    if let Some((final_orientation, nodes)) = solve_domain_beam(
        W,
        M,
        &grid,
        &initial_orientation,
